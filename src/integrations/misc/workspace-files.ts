...
export async function readWorkspaceFile(relativePath: string, options: { create?: boolean; ensureDirectory?: boolean; content?: string } = {}): Promise<ExtensionMessage> {
    try {
        // Get workspace root
...
export async function writeWorkspaceFile(relativePath: string, options: { create?: boolean; ensureDirectory?: boolean; content: string }): Promise<ExtensionMessage> {
    try {
        // Get workspace root
        const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
